...
export default function ListingConfigurator() {
  ...
  const fetchListings = async () => {
    ...
    try {
      ...
      setListings(data);
      // Auto-load the first listing if available
  ...
  // Load a specific listing into the form
  ...
  // Fetch a single listing by ID (used after save to get the server's version)
  const fetchListing = async (id) => {
    const res = await fetch(`${API_BASE}/listings/${id}`);
    return await res.json();
  };

  // Save: POST for new listings, PUT for existing ones
  const saveListing = async () => {
    ...
    try {
      ...
      // Refresh the sidebar list and load the saved listing
      await fetchListings();
      loadListing(saved);
      setSaveStatus("Saved ✓");
    } catch (err) {
